...
from mTCPIPConnections import cTCPIPConnection as cConnection, cTCPIPConnectionAcceptor as cConnectionAcceptor;
...
def fHandleNewConnectionOnServerSide(oConnectionAcceptor, oServerConnection):
  global bDisconnectionDetectedByServer;
  print "Server: %s." % (oServerConnection);
  print "Server waiting for bytes...";
  try:
    oServerConnection.fWaitUntilBytesAreAvailableForReading(60);
...
def fWait():
  ...
  try:
    oClientConnection.fWaitUntilBytesAreAvailableForReading(60);
